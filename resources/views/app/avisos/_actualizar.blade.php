<div id="modalMantenimientoAviso" class="modal modal-fill fade" data-backdrop="false" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form enctype="multipart/form-data" action="{{ route('empresa.update_aviso') }}" id="registroAviso" method="POST"
              data-ajax="true" data-close-modal="true" data-ajax-loading="#loading" data-ajax-success="OnSuccessRegistroAviso"
              data-ajax-failure="OnFailureRegistroAviso">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ $Aviso != null ? "Modificar" : " Registrar" }}  Oportunidad</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6" hidden>
                            <select name="modalidad_id" id="modalidad_id" class="form-input">
                                <option value="">Tipo de empleo</option>
                                @foreach($Modalidades as $q)
                                    <option value="{{ $q->id }}" {{ $Aviso != null && $Aviso->modalidad_id == $q->id ? "selected" : "" }}>{{ $q->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        {{-- aca tambien --}}
                        <div class="col-md-6" hidden>
                            <select name="horario_id" id="horario_id" class="form-input">
                                <option value="">Tiempo de empleo</option>
                                @foreach($Horarios as $q)
                                    <option value="{{ $q->id }}" {{ $Aviso != null && $Aviso->horario_id == $q->id ? "selected" : "" }}>{{ $q->nombre }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <input type="number" name="vacantes" class="form-input" placeholder="Cantidad de Vacantes" value="{{ $Aviso->vacantes }}" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        <div class="col-md-6">
                            <input type="text" name="periodo_vigencia" class="form-input" value="{{ $Aviso->periodo_vigencia }}" placeholder="Periodo de vigencia (Hasta cuando se aceptan postulaciones)" onfocus="(this.type='date')" onblur="(this.type='text')" required>
                        </div>
                        {{-- aca tambien --}}
                        <div class="col-md-6">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-bold btn-pure btn-primary">{{ $Aviso != null ? "Modificar" : " Registrar" }} Oportunidad</button>
                </div>
            </div>
        </form>
    </div>
...
<script type="text/javascript" src="{{ asset('app/js/avisos/_actualizar.min.js') }}"></script>
<script>
